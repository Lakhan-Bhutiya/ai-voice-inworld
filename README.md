...
ADMIN_USERNAME=admin                  # web UI login — leave blank to disable
ADMIN_PASSWORD=change-me
USER_USERNAME=user                    # optional second account, usage-only
USER_PASSWORD=change-me-too
```

### Login and roles
With any account configured, opening the app shows a sign-in screen; the
credentials are only compared server-side and a signed, HttpOnly cookie keeps
you signed in for a week. The cookie's signing secret comes from
`SESSION_SECRET`, or is generated once and stored in the local SQLite DB — so
sessions survive a restart either way. With no accounts configured, the UI is
open to anyone who can reach the port (the server logs a warning at startup).

| | admin | user |
|---|---|---|
| Studio, voices, cloning, history | ✅ | ✅ |
| Generations · characters · enhance calls | ✅ | ✅ |
| Estimated cost, published rates, calculator | ✅ | — |

Money is withheld server-side, not just hidden: `/api/usage` (and the totals
returned by synthesize/enhance) omit every cost field for a regular user, so
spend isn't readable from the API either. `APP_USERNAME`/`APP_PASSWORD` still
work as an alias for the admin pair.
...
| `/api/usage` | GET | `?sessionId=…` → persisted usage: session totals, all-time totals, per-model (costs admin-only) |
